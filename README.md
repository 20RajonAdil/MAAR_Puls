...
- **Shorts tab**: the public YouTube Data API v3 has no official "is this a Short" flag, so this is a heuristic, not a confirmed classification: a video counts as a Short when its duration is under 2 minutes. Thumbnail aspect ratio was tried as a second signal but dropped — in practice the API almost always returns thumbnails at fixed preset canvases (120x90, 320x180, 480x360, 1280x720) regardless of the source video's real orientation, even for genuine vertical Shorts, so requiring a square/vertical thumbnail silently rejected nearly everything. See `isLikelyShort` in `lib/utils.ts`. The Shorts tab UI says outright that this is estimated, since some short landscape videos can be mis-sorted in.
